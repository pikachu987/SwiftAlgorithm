...
#### - [N개의 퀸(N_Queens)](./2_Algorithm/11_BackTracking/N_Queens.md "N_Queens")
- N개의 열에 하나씩 퀸이 들어가지 않으면 가로로 위협
- 0번째 행에 0번째 열부터 마지막열까지 반복해서 Q를 넣음
- 다음행을 row라 할때 row의 0번째 열부터 마지막열까지 Q를 넣고
- 0번째 행에서 row행 전까지 row행의 Q가 위협받는지 검사
- 위협받으면 알고리즘이 성립되지 않음
- 마지막행까지 위협받지 않으면 알고리즘 성립
